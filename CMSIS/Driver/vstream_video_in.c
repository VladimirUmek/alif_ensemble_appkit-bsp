...
/* vStream Handle */
static StreamHandle_t hVideoIn;
...
static void DriverCPI_Callback(uint32_t cb_event) {
  ...
  if (cb_event & (ARM_CPI_EVENT_ERR_CAMERA_INPUT_FIFO_OVERRUN  |
...
static int32_t Start (uint32_t mode) {
  ...
  else if ((hVideoIn.idx_in == hVideoIn.idx_rel) && ((hVideoIn.flags & FLAGS_BUF_EMPTY) == 0)) {
    /* Buffer is empty */
    rval = VSTREAM_ERROR;
  }
  else if (hVideoIn.active == 1U) {
